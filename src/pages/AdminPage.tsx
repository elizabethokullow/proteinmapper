...
import { mockReports, mockReporters, getMarkets } from "@/lib/mockData";
import { Report, FOOD_TYPES, getAvailabilityLabel } from "@/lib/types";
import { Button } from "@/components/ui/button";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Check, Flag, Clock, Download, Users, FileText, AlertTriangle } from "lucide-react";
import { toast } from "sonner";

export default function AdminPage() {
  const [reports, setReports] = useState<Report[]>(mockReports);
  const markets = useMemo(() => getMarkets(reports), [reports]);

  const flaggedReports = reports.filter(r => r.validationStatus === "flagged");
  const pendingReports = reports.filter(r => r.validationStatus === "pending");

  const updateStatus = (id: string, status: Report["validationStatus"]) => {
    setReports(prev => prev.map(r => r.id === id ? { ...r, validationStatus: status } : r));
  ...
  const exportCSV = () => {
    ...
    const rows = reports.map(r =>
      `${r.id},${r.marketName},${r.foodType},${r.availability},${r.price},${r.reporterId},${r.timestamp},${r.validationStatus}`
    ).join("\n");
      ...
      <div className="container py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {[
            ...
            { label: "Reporters", value: mockReporters.length, icon: Users },
          ].map(s => (
            ...
            {reports.map(r => {
              ...
              return (
                <div key={r.id} className="rounded-lg border bg-card p-3 flex items-center gap-3 flex-wrap">
                  ...
                  <span className="font-medium text-sm min-w-[120px]">{r.marketName}</span>
                  ...
                  <span className="text-xs text-muted-foreground ml-auto">{r.reporterId}</span>
                  <span className="text-xs text-muted-foreground">{new Date(r.timestamp).toLocaleString()}</span>
                  {r.validationStatus !== "verified" && (
                    <Button variant="outline" size="sm" onClick={() => updateStatus(r.id, "verified")}>
            ...
            {flaggedReports.length === 0 ? (
              ...
              return (
                <div key={r.id} className="rounded-lg border border-destructive/30 bg-card p-3 flex items-center gap-3 flex-wrap">
                  ...
                  <span className="font-medium text-sm">{r.marketName}</span>
                  <span className="text-sm">{food?.emoji} {food?.label}</span>
                  <span className="text-xs text-muted-foreground ml-auto">{r.reporterId}</span>
                  <Button variant="outline" size="sm" onClick={() => updateStatus(r.id, "verified")}>
                    <Check className="h-3 w-3 mr-1" /> Verify
              ...
              <div key={rep.id} className="rounded-lg border bg-card p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {rep.totalReports} reports · {rep.verifiedReports} verified
                  </p>
                </div>
                <div className="text-right">
                  <div className="text-lg font-bold text-card-foreground">{(rep.trustScore * 100).toFixed(0)}%</div>
